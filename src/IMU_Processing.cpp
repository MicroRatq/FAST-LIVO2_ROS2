...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  if (!imu_time_init)
  {
    // double imu_time = rclcpp::Time(v_imu.front()->header.stamp).seconds() - first_lidar_time;
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  double t1, t2, t3;
  t1 = omp_get_wtime();
  // RCLCPP_ASSERT(lidar_meas.lidar != nullptr);
  if (!imu_en)
  {
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      //          <<stat.gravity[0]<<" "<<stat.gravity[1]<<" "<<stat.gravity[2]<<" "<<mean_acc.norm()<<"; acc covarience: "
      //          <<cov_acc[0]<<" "<<cov_acc[1]<<" "<<cov_acc[2]<<"; gry covarience: "
      ...
      //          <<cov_bias_acc[0]<<" "<<cov_bias_acc[1]<<" "<<cov_bias_acc[2]<<"; bg covarience: "
