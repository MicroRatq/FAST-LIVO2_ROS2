      ...
      extR(M3D::Identity())
{
  ...
  // Publish static transform from map to camera_init
  static tf2_ros::StaticTransformBroadcaster static_br(node_);
...
{
  node_->declare_parameter("common.lid_topic", std::string("/livox/lidar"));
  lid_topic = node_->get_parameter("common.lid_topic").as_string();
  node_->declare_parameter("common.imu_topic", std::string("/livox/imu"));
  imu_topic = node_->get_parameter("common.imu_topic").as_string();
  node_->declare_parameter("common.ros_driver_bug_fix", false);
  ros_driver_fix_en = node_->get_parameter("common.ros_driver_bug_fix").as_bool();
  node_->declare_parameter("common.img_en", 1);
  img_en = node_->get_parameter("common.img_en").as_int();
  node_->declare_parameter("common.lidar_en", 1);
  lidar_en = node_->get_parameter("common.lidar_en").as_int();
  node_->declare_parameter("common.img_topic", std::string("/left_camera/image"));
  img_topic = node_->get_parameter("common.img_topic").as_string();

  node_->declare_parameter("vio.normal_en", true);
  normal_en = node_->get_parameter("vio.normal_en").as_bool();
  node_->declare_parameter("vio.inverse_composition_en", false);
  inverse_composition_en = node_->get_parameter("vio.inverse_composition_en").as_bool();
  node_->declare_parameter("vio.max_iterations", 5);
  max_iterations = node_->get_parameter("vio.max_iterations").as_int();
  node_->declare_parameter("vio.img_point_cov", 100.0);
  IMG_POINT_COV = node_->get_parameter("vio.img_point_cov").as_double();
  node_->declare_parameter("vio.raycast_en", false);
  raycast_en = node_->get_parameter("vio.raycast_en").as_bool();
  node_->declare_parameter("vio.exposure_estimate_en", true);
  exposure_estimate_en = node_->get_parameter("vio.exposure_estimate_en").as_bool();
  node_->declare_parameter("vio.inv_expo_cov", 0.2);
  inv_expo_cov = node_->get_parameter("vio.inv_expo_cov").as_double();
  node_->declare_parameter("vio.grid_size", 5);
  grid_size = node_->get_parameter("vio.grid_size").as_int();
  node_->declare_parameter("vio.grid_n_height", 17);
  grid_n_height = node_->get_parameter("vio.grid_n_height").as_int();
  node_->declare_parameter("vio.patch_pyrimid_level", 3);
  patch_pyrimid_level = node_->get_parameter("vio.patch_pyrimid_level").as_int();
  node_->declare_parameter("vio.patch_size", 8);
  patch_size = node_->get_parameter("vio.patch_size").as_int();
  node_->declare_parameter("vio.outlier_threshold", 1000.0);
  outlier_threshold = node_->get_parameter("vio.outlier_threshold").as_double();

  node_->declare_parameter("time_offset.exposure_time_init", 0.0);
  exposure_time_init = node_->get_parameter("time_offset.exposure_time_init").as_double();
  node_->declare_parameter("time_offset.img_time_offset", 0.0);
  img_time_offset = node_->get_parameter("time_offset.img_time_offset").as_double();
  node_->declare_parameter("time_offset.imu_time_offset", 0.0);
  imu_time_offset = node_->get_parameter("time_offset.imu_time_offset").as_double();
  node_->declare_parameter("uav.imu_rate_odom", false);
  imu_prop_enable = node_->get_parameter("uav.imu_rate_odom").as_bool();
  node_->declare_parameter("uav.gravity_align_en", false);
  gravity_align_en = node_->get_parameter("uav.gravity_align_en").as_bool();

  node_->declare_parameter("evo.seq_name", std::string("01"));
  seq_name = node_->get_parameter("evo.seq_name").as_string();
  node_->declare_parameter("evo.pose_output_en", false);
  pose_output_en = node_->get_parameter("evo.pose_output_en").as_bool();
  node_->declare_parameter("imu.gyr_cov", 1.0);
  gyr_cov = node_->get_parameter("imu.gyr_cov").as_double();

  node_->declare_parameter("imu.acc_cov", 1.0);
  acc_cov = node_->get_parameter("imu.acc_cov").as_double();
  node_->declare_parameter("imu.imu_int_frame", 3);
  imu_int_frame = node_->get_parameter("imu.imu_int_frame").as_int();
  node_->declare_parameter("imu.imu_en", false);
  imu_en = node_->get_parameter("imu.imu_en").as_bool();
  node_->declare_parameter("imu.gravity_est_en", true);
  gravity_est_en = node_->get_parameter("imu.gravity_est_en").as_bool();
  node_->declare_parameter("imu.ba_bg_est_en", true);
  ba_bg_est_en = node_->get_parameter("imu.ba_bg_est_en").as_bool();

  node_->declare_parameter("preprocess.blind", 0.01);
  p_pre->blind = node_->get_parameter("preprocess.blind").as_double();
  node_->declare_parameter("preprocess.filter_size_surf", 0.5);
  filter_size_surf_min = node_->get_parameter("preprocess.filter_size_surf").as_double();
  int lidar_type_int;
  node_->declare_parameter("preprocess.lidar_type", static_cast<int>(AVIA));
  lidar_type_int = node_->get_parameter("preprocess.lidar_type").as_int();
  p_pre->lidar_type = static_cast<LID_TYPE>(lidar_type_int);
  node_->declare_parameter("preprocess.scan_line", 6);
  p_pre->N_SCANS = node_->get_parameter("preprocess.scan_line").as_int();
  node_->declare_parameter("preprocess.point_filter_num", 3);
  p_pre->point_filter_num = node_->get_parameter("preprocess.point_filter_num").as_int();
  node_->declare_parameter("preprocess.feature_extract_enabled", false);
  p_pre->feature_enabled = node_->get_parameter("preprocess.feature_extract_enabled").as_bool();

  node_->declare_parameter("pcd_save.interval", -1);
  pcd_save_interval = node_->get_parameter("pcd_save.interval").as_int();
  node_->declare_parameter("pcd_save.pcd_save_en", false);
  pcd_save_en = node_->get_parameter("pcd_save.pcd_save_en").as_bool();
  node_->declare_parameter("pcd_save.colmap_output_en", false);
  colmap_output_en = node_->get_parameter("pcd_save.colmap_output_en").as_bool();
  node_->declare_parameter("pcd_save.filter_size_pcd", 0.5);
  filter_size_pcd = node_->get_parameter("pcd_save.filter_size_pcd").as_double();
  node_->declare_parameter("extrin_calib.extrinsic_T", std::vector<double>());
  extrinT = node_->get_parameter("extrin_calib.extrinsic_T").as_double_array();
  node_->declare_parameter("extrin_calib.extrinsic_R", std::vector<double>());
  extrinR = node_->get_parameter("extrin_calib.extrinsic_R").as_double_array();
  node_->declare_parameter("extrin_calib.Pcl", std::vector<double>());
  cameraextrinT = node_->get_parameter("extrin_calib.Pcl").as_double_array();
  node_->declare_parameter("extrin_calib.Rcl", std::vector<double>());
  cameraextrinR = node_->get_parameter("extrin_calib.Rcl").as_double_array();
  node_->declare_parameter("debug.plot_time", -10.0);
  plot_time = node_->get_parameter("debug.plot_time").as_double();
  node_->declare_parameter("debug.frame_cnt", 6);
  frame_cnt = node_->get_parameter("debug.frame_cnt").as_int();

  node_->declare_parameter("publish.blind_rgb_points", 0.01);
  blind_rgb_points = node_->get_parameter("publish.blind_rgb_points").as_double();
  node_->declare_parameter("publish.pub_scan_num", 1);
  pub_scan_num = node_->get_parameter("publish.pub_scan_num").as_int();
  node_->declare_parameter("publish.pub_effect_point_en", false);
  pub_effect_point_en = node_->get_parameter("publish.pub_effect_point_en").as_bool();
  node_->declare_parameter("publish.dense_map_en", false);
  dense_map_en = node_->get_parameter("publish.dense_map_en").as_bool();

  // Camera parameters
  node_->declare_parameter("cam_model", std::string("Pinhole"));
  cam_model = node_->get_parameter("cam_model").as_string();
  node_->declare_parameter("cam_width", 1280);
  cam_width = node_->get_parameter("cam_width").as_int();
  node_->declare_parameter("cam_height", 1024);
  cam_height = node_->get_parameter("cam_height").as_int();
  node_->declare_parameter("cam_fx", 1293.56944);
  cam_fx = node_->get_parameter("cam_fx").as_double();
  node_->declare_parameter("cam_fy", 1293.3155);
  cam_fy = node_->get_parameter("cam_fy").as_double();
  node_->declare_parameter("cam_cx", 626.91359);
  cam_cx = node_->get_parameter("cam_cx").as_double();
  node_->declare_parameter("cam_cy", 522.799224);
  cam_cy = node_->get_parameter("cam_cy").as_double();
  node_->declare_parameter("cam_d0", -0.076160);
  cam_d0 = node_->get_parameter("cam_d0").as_double();
  node_->declare_parameter("cam_d1", 0.123001);
  cam_d1 = node_->get_parameter("cam_d1").as_double();
  node_->declare_parameter("cam_d2", -0.00113);
  cam_d2 = node_->get_parameter("cam_d2").as_double();
  node_->declare_parameter("cam_d3", 0.000251);
  cam_d3 = node_->get_parameter("cam_d3").as_double();
  node_->declare_parameter("scale", 0.5);
...
void LIVMapper::initializeComponents() 
{
  ...
  // Manually create camera object instead of using vikit's loadFromRosNs
  if (cam_model == "Pinhole") {
    vio_manager->cam = new vk::PinholeCamera(cam_width, cam_height, cam_fx, cam_fy, cam_cx, cam_cy, cam_d0, cam_d1, cam_d2, cam_d3);
  } else {
    throw std::runtime_error("Camera model not correctly specified. Only Pinhole model is supported.");
  }
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr node, image_transport::ImageTransport &it) 
{
  if (p_pre->lidar_type == AVIA) {
    sub_pcl = node_->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, 200000, std::bind(&LIVMapper::livox_pcl_cbk, this, std::placeholders::_1));
  } else {
    sub_pcl = node_->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, 200000, std::bind(&LIVMapper::standard_pcl_cbk, this, std::placeholders::_1));
  }
  sub_imu = node_->create_subscription<sensor_msgs::msg::Imu>(imu_topic, 200000, std::bind(&LIVMapper::imu_cbk, this, std::placeholders::_1));
  sub_img = node_->create_subscription<sensor_msgs::msg::CompressedImage>(img_topic, 200000, std::bind(&LIVMapper::img_cbk, this, std::placeholders::_1));
  ...
  pubSubVisualMap = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/cloud_visual_sub_map_before", 100);
...
{
  // std::cout << "stateEstimationAndMapping" << std::endl;
...
{    
  ...
  euler_cur = RotMtoEuler(_state.rot_end);
  geoQuat = tf2::toMsg(tf2::Quaternion(tf2::Vector3(0, 0, 1), euler_cur(2)) * 
                      tf2::Quaternion(tf2::Vector3(0, 1, 0), euler_cur(1)) * 
                      tf2::Quaternion(tf2::Vector3(1, 0, 0), euler_cur(0)));
...
void LIVMapper::run() 
{
  ...
  while (rclcpp::ok()) 
  {
    ...
    {
      // std::cout << "Sync packages failed" << std::endl;
      rate.sleep();
      continue;
    }   
    handleFirstFrame();
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  //   sync_jump_flag = true;
  //   msg->header.stamp = rclcpp::Time(last_timestamp_lidar + 0.1);
  // }
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // Apply time offset to IMU timestamp and convert to ROS2 time
  double timestamp = rclcpp::Time(msg_in->header.stamp).seconds() - imu_time_offset;
  ...
  if (fabs(last_timestamp_lidar - timestamp) > 0.5 && (!ros_driver_fix_en))
  {
    ...
    std::cout << "last_timestamp_lidar: " << std::fixed << std::setprecision(6) 
              << last_timestamp_lidar << " timestamp: " << std::fixed << std::setprecision(6) << timestamp << std::endl;
  }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::CompressedImage::ConstSharedPtr &msg_in)
{
  ...
  // Convert compressed image to regular image
  sensor_msgs::msg::Image::SharedPtr msg(new sensor_msgs::msg::Image());
  msg->header = msg_in->header;
  msg->encoding = "bgr8";
  msg->width = 0;  // Will be set after decompression
  msg->height = 0; // Will be set after decompression
  msg->step = 0;   // Will be set after decompression
  
  // Decompress the image
  cv::Mat compressed_img = cv::imdecode(cv::Mat(msg_in->data), cv::IMREAD_COLOR);
  if (compressed_img.empty()) {
    RCLCPP_ERROR(node_->get_logger(), "Failed to decompress image");
    return;
  }
  
  // Convert to sensor_msgs::msg::Image
  cv_bridge::CvImage cv_img;
  cv_img.header = msg_in->header;
  cv_img.encoding = "bgr8";
  cv_img.image = compressed_img;
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  {
    // std::cout << "lid_raw_data_buffer.empty()" << lid_raw_data_buffer.size() << "lidar_en" << lidar_en << std::endl;
    return false;
  }
  if (img_buffer.empty() && img_en)
  {
    // std::cout << "img_buffer.empty()" << img_buffer.size() << "img_en" << img_en << std::endl;
    return false;
  }
  if (imu_buffer.empty() && imu_en)
  {
    // std::cout << "imu_buffer.empty() && imu_en" << imu_buffer.size() << "imu_en" << imu_en << std::endl;
    return false;
  }
  ...
  case ONLY_LIO:
  {
    ...
    { // waiting imu message needs to be
      ...
      std::cout << "Waiting imu message needs to be larger than lidar frame end time" << std::endl;
      std::cout << "last_timestamp_imu: " << last_timestamp_imu << std::endl;
      std::cout << "meas.lidar_frame_end_time: " << meas.lidar_frame_end_time << std::endl;
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      if (img_capture_time > lid_newest_time || img_capture_time > imu_newest_time)
      {
        ...
        // std::cout << "lost first camera frame" << std::endl;
        // std::cout << "img_capture_time, lid_newest_time, imu_newest_time: " 
        //           << std::fixed << std::setprecision(6) 
        //           << img_capture_time << " " << lid_newest_time << " " << imu_newest_time << std::endl;
...
void LIVMapper::publish_frame_world(const std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
...
void LIVMapper::publish_visual_sub_map(const std::shared_ptr<rclcpp::Publisher<visualization_msgs::msg::MarkerArray>> &pubSubVisualMap)
{
  ...
  *sub_pcl_visual_map_pub = *laserCloudFullRes;
  if (1)
  {
    ...
    // Convert pointcloud to marker array for visualization
    visualization_msgs::msg::Marker marker;
    marker.header.stamp = node_->now();
    marker.header.frame_id = "camera_init";
    marker.ns = "visual_sub_map";
    marker.id = 0;
    marker.type = visualization_msgs::msg::Marker::POINTS;
    marker.action = visualization_msgs::msg::Marker::ADD;
    
    for (const auto& point : sub_pcl_visual_map_pub->points) {
    ...
    pubSubVisualMap->publish(marker_array);
  }
}

void LIVMapper::publish_effect_world(const std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list)
{
...
void LIVMapper::publish_odometry(const std::shared_ptr<rclcpp::Publisher<nav_msgs::msg::Odometry>> &pubOdomAftMapped)
{
  odomAftMapped.header.frame_id = "camera_init";
  odomAftMapped.child_frame_id = "aft_mapped";
  odomAftMapped.header.stamp = node_->now(); //.ros::Time()fromSec(last_timestamp_lidar);
...
void LIVMapper::publish_mavros(const std::shared_ptr<rclcpp::Publisher<geometry_msgs::msg::PoseStamped>> &mavros_pose_publisher)
{
...
void LIVMapper::publish_path(const std::shared_ptr<rclcpp::Publisher<nav_msgs::msg::Path>> pubPath)
{
  set_posestamp(msg_body_pose.pose);
  msg_body_pose.header.stamp = node_->now();
  msg_body_pose.header.frame_id = "camera_init";
